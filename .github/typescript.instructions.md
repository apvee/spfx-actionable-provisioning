---
name: 'TypeScript 5.x / ES2022 Development Instructions'
...
> Follow the repository's actual TypeScript configuration. In this repo, `tsconfig.json` targets ES2018 for source-checking, and `tsconfig.build.json` currently emits CommonJS for the published build.
...
- Target TypeScript 5.x and prefer native features over polyfills.
- Use ES module syntax (`import`/`export`) in source code; do not write CommonJS (`require`, `module.exports`) by hand.
- The emitted module format is controlled by the repo `tsconfig*` files; don't change it unless the task explicitly requires a packaging change.
